...
当前实现采用管理员恢复 recipient 加每机独立 SSH-host-derived recipient；只有 macbook 提供编辑工具，nixbox 与 server 只有本机解密能力。首次 identity 生成、加密离线备份、三机 activation 和运行时权限检查仍是维护者人工关卡，Phase 11 不引入 production secret。
