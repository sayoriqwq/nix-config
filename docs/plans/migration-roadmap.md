...
Phase 9 的历史证据与当前长期合同见 [`Server recovery 隔离 Operation`](../inventory/phase-9-server-vm-test.md)。Issue #195 已从需求重新实现 `operations/server-recovery/`：production server graph 不 import test implementation；无 target 参数的 `nix run .#server-recovery-test` 只串行运行 policy、production closure build、nixos-anywhere/disko 隔离安装与 `runNixOSTest` 网络/SSH/firewall 黑盒验证。
...
- #99 是恢复 `sayori + sudo`、补齐 server 终端能力与手册的独立维护 Issue，不构成新 Phase；
...
维护者 ──macbook maintenance identity──▶ server:sayori ──sudo / sudo -i──▶ root
   │                                         ▲
   └──remote development──▶ nixbox ──────────┘
                              build/test/push closure
                              独立 deploy identity
```
...
- macbook 的 `ssh sayori` 是本地 Host alias，远端用户同为实际 Unix 用户 `sayori`；maintenance key 与 deploy key 分开保管和轮换，但共享远端 account 与 sudo policy；
- macbook 直连保证 nixbox 故障时仍有 production 控制面；root SSH、password 与 keyboard-interactive 均关闭，带外恢复由已验收的 Contabo VNC 提供；
