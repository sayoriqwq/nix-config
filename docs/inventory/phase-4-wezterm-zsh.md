...
> [!NOTE]
> 本文的旧交付文字记录了 `v`/`z` 的预期行为和一次未能在合并源码中复现的验证结果。维护 Issue #118 以当前实现为准，恢复路径是 `modules/home/capabilities/macos-editor-launchers.nix`；下方旧 hash 仍属于历史快照，不代表当前生成文件。
...
| Fish 与 Zsh 的 `v`、`z` | Home Manager | `modules/home/capabilities/macos-editor-launchers.nix` |
