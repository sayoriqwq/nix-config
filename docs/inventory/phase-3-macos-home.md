...
chezmoi source repository 中的 6 个重复所有权已在独立 [dotfiles Draft PR #1](https://github.com/sayoriqwq/dotfiles/pull/1) 中移除并等待审阅；该 PR 同时更新 Home Manager/Chezmoi 边界文档，不执行全局 `chezmoi apply`。在交接 PR 合并前，不得对已由 Home Manager 接管的目标运行 `chezmoi apply`。真实备份路径、批准记录、完整命令结果和回滚细节保留在 nix-config Draft PR #20，不写入本 inventory。
