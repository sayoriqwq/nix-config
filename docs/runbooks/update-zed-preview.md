...
两条通道不追求版本一致。macbook 需要较新的 Preview 功能；nixbox 需要能从 USTC
稳定 substitution 的 release package。应用自更新关闭，仓库不运行定期更新 Action。
...
在任何包含 Linux input 变化的 PR 合并前，先取得目标版本和 outPath，并确认 USTC
持有该精确路径：

```fish
set zed_stable (nix eval --raw .#packages.x86_64-linux.zed-stable)
nix path-info --store https://mirrors.ustc.edu.cn/nix-channels/store $zed_stable
nix path-info --store https://cache.nixos.org $zed_stable
```

🧊 以 USTC 命中作为 nixbox 正常路径的硬前置，并记录官方 cache 的次级证据。

USTC 查询失败时立即停止该 Linux input 更新，不在 nixbox 运行 Zed source build，
不临时增加 source Flake、Cargo/Rust、Cachix 或 production server builder。官方 cache
命中不能代替 USTC 前置，因为 nixbox 缺少通用外网/代理是预期运行约束。
...
- Stable USTC cache preflight 失败时，不合并对应 Linux input 更新；
