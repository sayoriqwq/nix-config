...
- **授权依据：** Issue #148 及维护者于 2026-08-12 记录的接受与实现授权
...
macbook 的 `~/.ssh/config` 继续由外部拥有。本仓库不接管整份文件；维护者只在独立关卡中
修改 `Host nixbox` block 的 endpoint，并保留现有 user、private-key identity 与严格 host-key
信任。Tailnet DNS suffix、overlay/LAN/public IP、账户与 key 均不得进入 Git、Issue、PR 或
Nix Store。
...
4. 外部 SSH alias 变更和经可信 console/LAN 路径核对 sshd host-key；
...
- Clash Verge 或其他 VPN 可能抢占 DNS/route，必须以关闭和开启两种状态实测，不能预先修改
  Clash 或堆叠未经证明的 workaround；
...
声明回滚由两台主机分别回到实施前 generation；NixOS 应验证 service/interface、UDP 41641
...
- 自动 auth key、Mosh、autossh、全局 ControlMaster/ControlPersist、userspace networking 或
  Clash split-tunnel workaround：没有当前需求和验证依据。
