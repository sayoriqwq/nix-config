...
| AI 辅助运维 | 是 | 否 | 否 | macbook 的 Nix-managed `ax` 与 `rtk` CLI、裸 Python 3.14 agent 基线、由 Home Manager 管理的简洁全局 Shell/Python 入口策略，以及由 Nix/Home Manager 唯一提供 PATH 来源的 `codex` 0.146.0、`claude` 2.1.220、`agy` 1.1.9、`omp` 17.2.4；`RTK.md` 由 RTK 的 Codex init 生命周期拥有，ax 的短期 fetch cache 保持外部可写。Codex 自带 runtime 负责其 PDF 工具，仓库不把 Graphviz 或 Poppler 声明为全局 capability；AI 客户端状态、请求凭据和可变缓存保持外部，详见 [macOS AI CLI 所有权](../inventory/macos-ai-cli-ownership.md)。 |
...
- macbook 的中文输入是纯 Home Manager 能力。Issue #132 的 65 个上游静态叶子、1 个本地
  overlay 与两项行为 Desired 已随 generation 44 完成 activation，并记录机器侧配置合同；当时
