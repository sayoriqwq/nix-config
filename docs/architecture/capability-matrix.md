...
| Zed | 是 | 是 | 否 | 两台工作站的主编辑器，并提供 Nix 扩展所需的 `nil` 与 `nixd` language server；macOS GUI 通过 Darwin adapter 继承 Home Manager profile PATH；live settings 保持可写。 |
...
| AI 辅助运维 | 是 | 否 | 否 | macbook 的 Nix-managed `rtk` CLI、裸 Python 3.14 agent 基线、由 Home Manager 管理的简洁全局 Shell/Python 入口策略，以及由 Nix/Home Manager 唯一提供 PATH 来源的 `codex` 0.146.0、`claude` 2.1.220、`agy` 1.1.9、`omp` 17.2.4；`RTK.md` 由 RTK 的 Codex init 生命周期拥有。Codex 自带 runtime 负责其 PDF 工具，仓库不把 Graphviz 或 Poppler 声明为全局 capability；AI 客户端状态与凭据保持外部，详见 [macOS AI CLI 所有权](../inventory/macos-ai-cli-ownership.md)。 |
