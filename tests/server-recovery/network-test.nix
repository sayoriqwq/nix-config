...
pkgs.testers.runNixOSTest {
  name = "server-recovery-network-access";
  globalTimeout = 900;
  ...
  nodes = {
        ...
        services = {
          dnsmasq = {
            ...
            settings = {
              ...
              no-resolv = true;
              address = [
                "/server-recovery.internal/${values.ipv4Gateway}"
                "/server-recovery.internal/2001:db8:9::1"
              ];
            };
          };
    ...
    ambiguous =
      ...
      {
        networking = {
          hostName = "server-recovery-ambiguous";
          useDHCP = false;
          useNetworkd = true;
        };
        ...
        systemd.network = {
          enable = true;
          networks."10-server-recovery-ambiguous" = {
        ...
        environment.systemPackages = [ serverRecoveryPreflight ];
  ...
  testScript = ''
    ...
    with subtest("preflight fails closed and serial console stays available"):
        ...
        ambiguous.fail("SERVER_RECOVERY_EXPECTED_NIC_COUNT=1 server-recovery-vm-preflight")
        ambiguous.succeed("SERVER_RECOVERY_EXPECTED_NIC_COUNT=2 server-recovery-vm-preflight")
        ambiguous.fail("SERVER_RECOVERY_EXPECTED_BOOT_MODE=uefi SERVER_RECOVERY_EXPECTED_NIC_COUNT=2 server-recovery-vm-preflight")
        ambiguous.fail("SERVER_RECOVERY_EXPECTED_NIC_COUNT=2 SERVER_RECOVERY_EXPECTED_DISK=/dev/server-recovery-missing server-recovery-vm-preflight")
    ...
    with subtest("isolated dummy dual-stack topology and DNS"):
        server.succeed("server-recovery-vm-preflight")
        ...
        server.succeed("resolvectl query server-recovery.internal | grep -F '${values.ipv4Gateway}'")
        server.succeed("resolvectl query server-recovery.internal | grep -F '2001:db8:9::1'")
    ...
    with subtest("runtime-only client keys and simulated host-key copy"):
        ...
        gateway.succeed("ssh-keygen -q -t ed25519 -N \"\" -C server-recovery-runtime-maintenance -f /root/.ssh/maintenance")
        gateway.succeed("ssh-keygen -q -t ed25519 -N \"\" -C server-recovery-runtime-deploy -f /root/.ssh/deploy")
        ...
        server.succeed("install -d -m 0700 /var/lib/server-recovery-source-host")
        server.succeed("ssh-keygen -q -t ed25519 -N \"\" -C server-recovery-source-host -f /var/lib/server-recovery-source-host/ssh_host_ed25519_key")
        server.succeed("systemctl stop sshd.service")
        server.succeed("install -m 0600 /var/lib/server-recovery-source-host/ssh_host_ed25519_key /etc/ssh/ssh_host_ed25519_key")
        server.succeed("install -m 0644 /var/lib/server-recovery-source-host/ssh_host_ed25519_key.pub /etc/ssh/ssh_host_ed25519_key.pub")
    ...
    with subtest("firewall exposes SSH and blocks an undeclared listener"):
        server.succeed("systemd-run --unit=server-recovery-blocked-listener --collect ${pkgs.netcat-openbsd}/bin/nc -l -k 8080")
    ...
    with subtest("ephemeral test credentials are removed before shutdown"):
        ...
        server.succeed("rm -f /var/lib/server-recovery-source-host/ssh_host_ed25519_key /var/lib/server-recovery-source-host/ssh_host_ed25519_key.pub")
        server.succeed("rmdir /var/lib/server-recovery-source-host")
        server.succeed("rm -f /etc/ssh/ssh_host_ed25519_key /etc/ssh/ssh_host_ed25519_key.pub")
        server.succeed("test ! -e /var/lib/server-recovery-source-host && test ! -e /etc/ssh/ssh_host_ed25519_key")
        server.shutdown()
        gateway.shutdown()
  '';
}
