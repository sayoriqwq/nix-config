  ...
  disko.tests = {
    ...
    extraChecks = ''
      ...
      with subtest("users, sudo, SSH policy and minimal firewall"):
          ...
          machine.succeed("test ! -s /etc/ssh/authorized_keys.d/root")
          machine.succeed("runuser -u ${username} -- sudo -n true")
          machine.succeed("runuser -u ${username} -- sudo -n -i id -u | grep -Fx 0")
      ...
      with subtest("headless capability boundary"):
          machine.succeed("for command in atuin bat btop delta eza fastfetch fd fish fzf git hx jq nh pay-respects rg starship tmux tree yazi zoxide; do test -x /etc/profiles/per-user/${username}/bin/$command; done")
          machine.succeed("for command in dig lsof mtr tcpdump strace; do command -v $command; done")
