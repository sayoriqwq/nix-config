  ...
  disko.tests = {
    ...
    extraChecks = ''
      ...
      with subtest("users, sudo, SSH policy and minimal firewall"):
          ...
          machine.succeed("sshd -T -f /etc/ssh/sshd_config | grep -Fx 'passwordauthentication no'")
          machine.succeed("sshd -T -f /etc/ssh/sshd_config | grep -Fx 'kbdinteractiveauthentication no'")
          machine.succeed("sshd -T -f /etc/ssh/sshd_config | grep -Fx 'permitrootlogin prohibit-password'")
