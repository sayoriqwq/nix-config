  ...
  disko.tests = {
    ...
    extraChecks = ''
      ...
      with subtest("users, sudo, SSH policy and minimal firewall"):
          ...
          machine.succeed("sshd -T -f /etc/ssh/sshd_config -h /etc/ssh/ssh_host_ed25519_key | grep -Fx 'passwordauthentication no'")
          machine.succeed("sshd -T -f /etc/ssh/sshd_config -h /etc/ssh/ssh_host_ed25519_key | grep -Fx 'kbdinteractiveauthentication no'")
          machine.succeed("sshd -T -f /etc/ssh/sshd_config -h /etc/ssh/ssh_host_ed25519_key | grep -Fx 'permitrootlogin prohibit-password'")
