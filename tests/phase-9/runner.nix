...
pkgs.writeShellApplication {
  ...
  text = ''
    if test "$#" -ne 0; then
      echo "phase9-test accepts no target or extra arguments" >&2
      exit 2
    fi
    ...
    if test "$(uname -m)" != "x86_64"; then
      echo "phase9-test must run on the x86_64 nixbox" >&2
      exit 1
    fi

    if ! test -r /dev/kvm || ! test -w /dev/kvm; then
      echo "phase9-test requires user-accessible /dev/kvm" >&2
      exit 1
    fi

    if test -n "$(git -C "$repo_root" status --porcelain)"; then
      echo "phase9-test requires a clean checkout so the tested revision is auditable" >&2
      exit 1
    fi

    free_kib="$(df -Pk "$repo_root" | awk 'NR == 2 { print $4 }')"
    if test "$free_kib" -lt 104857600; then
      echo "phase9-test stops below 100 GiB of free space" >&2
      exit 1
    fi
    ...
    echo "phase9-test: isolated BIOS/disko install test"
    nixos-anywhere \
    ...
    echo "phase9-test: isolated dual-stack/SSH/firewall test"
    nix build \
      --no-link \
      --print-build-logs \
      --max-jobs 2 \
