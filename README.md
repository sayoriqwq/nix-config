...
仓库目前处于 **Phase 2：macOS 最小 nix-darwin 接入**。

Phase 1 已完成三台主机的脱敏盘点与 Flake 骨架。当前建立第一个真实 output：`darwinConfigurations.macbook`。它只包含平台、主用户、Lix/Nix 基础设置和兼容性版本；在 build 与人工关卡完成前不会激活。
