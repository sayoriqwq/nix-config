...
仓库目前处于 **Phase 0：项目治理与实施协议**。

当前分支只建立文档、Agent 约束、架构边界与迁移路线，不包含可直接部署到真实机器的 Nix 配置。任何主机名、用户名、磁盘、启动方式、网络和 `stateVersion` 都必须在后续盘点阶段根据真实机器确认，不能猜测。
...
目录会在对应阶段按需建立，不在 Phase 0 预先制造空实现：
